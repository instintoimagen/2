export default function scrollSpy() {
  ...
  // La callBack recibe entradas, las cuales son los elementos que están ingresando en la visualización de view port (navegador). Las Entries en este ejemplo son las esas section con el data attribute. CallBack no siempre refiere a asincronía, en este caso es porque Intersection Observer está esperando recibir una función.
  const cb = (entries) => {
  ...
  // Intersection Observer es una API que detecta cuando en la parte visible del navegador (View Port) se encuentra un elemento. Recibe una cb (callBack - en este caso es la función que espera recibir) y propiedades, en esas propiedades del Intersection Observer podemos estabrecer el margen de visualización, para que no marque dos al mismo tiempo.
  const observer = new IntersectionObserver(cb, {
    ...
    // Con la propiedad threshold establecemos un límite, con valores de 0 a 1. Por ejemplo: si el valor es 0.5 y el elemento se visualiza por lo menos un 50% entonces dará positivo y se marcará. [Entre corchetes pueden ir valores de min y max].
    threshold: [0.5, 0.75],
  });
  ...
  // A los elemento que están en la variable $secciones: a cada uno ejecuta la función observer, con el método .observe() y recibe como parámetro el elemento que va a observar, el mismo elemento al que está recorriendo.
}
